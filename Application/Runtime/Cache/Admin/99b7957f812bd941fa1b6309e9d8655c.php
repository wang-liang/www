...
<body >
    <form action="/index.php/admin/index/edit/id/202" method="post">
		<?php foreach ($data as $k=>$v): ?>
        <div>标题：<input type="text" name="title" style="width: 500px" value="<?php echo $v['title']?>"></div>
		<div style="margin-top:5px;">分类：<select name="cat_id">
					<option value="0">请选择</option>
					<?php foreach ($catData as $k1 => $v1): if($v['cat_id'] == $v1['cat_id']) $select = 'selected="selected"'; else $select = ''; ?>
					<option <?php echo $select; ?> value="<?php echo $v1['cat_id']; ?>"><?php echo str_repeat('-', 6*$v1['level']) . $v1['cat_name']; ?></option>
					<?php endforeach; ?>
				</select></div>
        <div style="margin-top:5px;">内容：</div>
        <div>
			<!--<textarea rows="10" cols="50" id="answer" name="answer" style='width:550px;height:260px;'></textarea> -->
			<textarea name="content" rows="10" cols="50" style="width:700px;height:260px;visibility:hidden;"><?php echo $v['content']?></textarea>
		</div>
